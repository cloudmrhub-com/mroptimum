...
try
    ...
    for ks=1:numel(signalfilename)
        ...
        if ischar(thefilename)
            ...
            S=cm2DRawDataReader();
            %on the 5D there's the re[etitions so cicle and set the kspace
            %accordingly
            for repli=1:size(thefilename,5)
            S.setcm2DRawDataReaderImageKSpaceFrom2DSlice(thefilename(:,:,:,:,repli),1,1,repli);
            end
